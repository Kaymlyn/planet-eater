...
/**
 * FIXED VERSION - Addresses round-trip positioning bug
 *
 * The bug was in calculateOrbitalState() which called calculateOrbitAfterT0()
 * even when no time had elapsed. This caused unnecessary round-trips through
 * anomaly conversions (true → mean → eccentric → true), introducing numerical error.
 *
 * FIX: When elapsedSinceEpoch == 0, directly compute position/velocity from
 * the stored trueAnomaly without any time propagation.
 *
...
public record Orbit(double semiMajorAxis,
    ...
    /**
     * NEW METHOD: Direct calculation of orbital state from stored elements
     *
     * This method computes position and velocity directly from the stored
     * trueAnomaly without any time propagation or anomaly conversions.
     *
     * Used when elapsedSinceEpoch == 0 to ensure perfect round-trip accuracy.
     */
    private OrbitalState calculateOrbitalStateDirectly() {
        ...
        return new OrbitalState(position3D, velocity3D, this);
    }
     ...
     * star.
     *
     * @param timeElapsed Time since epoch
     * @return OrbitalState at the specified time
     */
    ...
    private double conservedAngularMomentum() {
        return Math.sqrt(centerBody.getGravitationalParameter() * semiMajorAxis * (1.0 - eccentricity * eccentricity));
    }
