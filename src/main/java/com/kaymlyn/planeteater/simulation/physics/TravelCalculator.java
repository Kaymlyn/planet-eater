...
public class TravelCalculator {
     ...
     * IMPROVED: Now handles elliptical and inclined orbits
     */
    public static Trajectory calculateRendezvousV1(Vector3D startPos, Vector3D startVel,
                                                   Orbiter target,
                                                   Spacecraft spacecraft,
                                                   OrbitalSystem system) {
        System.out.println("Loc : " + startPos);
        System.out.println("Vel : " + startVel);
        System.out.println("Des : " + target);
        System.out.println("SpacecraftVel : " + spacecraft.getVelocity());
        System.out.println("Gravitational? : " + (spacecraft.getLocation() instanceof Gravitational));
        System.out.println("SpacecraftLocVel : " + spacecraft.getLocation().getVelocity());
        System.out.println("SpacecraftPos : " + spacecraft.getPosition());
        OrbitInitializer targetElements;
        // Get target's orbital elements
        Vector3D initialVelocity;
        if(spacecraft.getVelocity() == Vector3D.ZERO ) {
            if(spacecraft.getLocation() instanceof Gravitational) {
                initialVelocity = Gravitational.getCircularOrbitVector((Gravitational)spacecraft.getLocation(),
                        spacecraft.getLocation().getPosition().distanceTo(spacecraft.getLocation().getPosition().add(Vector3D.randomUnitVector())));
            } else {
                initialVelocity = spacecraft.getLocation().getVelocity();
            }
        } else {
            initialVelocity = spacecraft.getVelocity();
        }
        if(target instanceof Gravitational) {
            System.out.println("Target Grav");
            System.out.println("Initial Velocity : " + initialVelocity);
            targetElements = OrbitInitializer.calculateOrbitalInitializer((Gravitational) target,spacecraft.getPosition(), initialVelocity);
        } else {
            System.out.println("Target Not Grav");
            targetElements = OrbitInitializer.calculateOrbitalInitializer(
                    new Gravitational() {
                        @Override
        ...
        double i = targetElements.inclination();
        ...
        System.out.println("distance " + distance + " deltaV " + estimatedDeltaV + " estimatedTime " + estimatedTime);

        // Calculate phase angles
        double spacecraftAngle = Math.atan2(startPos.getY(), startPos.getX());
        double targetAngle = Math.atan2(currentTargetPos.getY(), currentTargetPos.getX());
        double currentPhaseAngle = targetAngle - spacecraftAngle;

        while (currentPhaseAngle < 0) currentPhaseAngle += 2 * Math.PI;
        while (currentPhaseAngle >= 2 * Math.PI) currentPhaseAngle -= 2 * Math.PI;


        // Predict target position using proper orbital mechanics
        Vector3D predictedTargetPos = predictOrbitalPosition(targetElements,
                estimatedTime, system);

        OrbitInitializer startingOrbit;
        if(spacecraft.getLocation() instanceof Gravitational) {
            startingOrbit = OrbitInitializer.calculateOrbitalInitializer((Gravitational) spacecraft.getLocation(), startPos, startVel);
        } else {
            startingOrbit = OrbitInitializer.calculateOrbitalInitializer(new Gravitational() {
                @Override
                public double getMass() {
                    return spacecraft.getLocation().getMass();
                }

                @Override
                public Vector3D getVelocity() {
                    return spacecraft.getLocation().getVelocity();
                }
        ...
        // Calculate trajectory to predicted position

        Trajectory basicTrajectory = calculateGeneralTransfer(startingOrbit, targetElements, system.getCentralStar().getMass(),
                spacecraft);

        System.out.println(basicTrajectory.travelTime);

        // Add plane change delta-v if orbits are inclined
        ...
        double deltaInclination = Math.abs(i - startInclination);

        double planeChangeDV = 0.0;
        if (deltaInclination > 0.001) { // More than ~0.06 degrees
            double velocity = startVel.magnitude();
            planeChangeDV = calculatePlaneChangeDeltaV(velocity, deltaInclination);
        }

        // Total delta-v including plane change
        double totalDeltaV = basicTrajectory.deltaV + planeChangeDV;
        double totalFuel = spacecraft.getFuelRequiredForDeltaV(totalDeltaV);

        return new Trajectory(startPos, predictedTargetPos, totalDeltaV,
                basicTrajectory.travelTime, totalFuel, targetVelocity.magnitude());
    }
    ...
    public static Trajectory calculateRendezvous(Vector3D startPos, Vector3D startVel,
                                                 ...
                                                 Spacecraft spacecraft,
                                                 OrbitalSystem system) {
        OrbitInitializer targetOrbit = system.calculateOrbitalElements(target);
        double a = targetOrbit.semiMajorAxis();
        double e = targetOrbit.eccentricity();
        double i = targetOrbit.inclination();

        double mu = PhysicsConstants.G * system.getCentralStar().getMass();

        // Calculate target's orbital period
        double targetPeriod = 2 * Math.PI * Math.sqrt(Math.pow(a, 3) / mu);
        ...
        double r2 = a;
        double transferTime = Math.PI * Math.sqrt(Math.pow((r1 + r2) / 2, 3) / mu);

        double targetAngularVelocity = Math.sqrt(mu / Math.pow(a, 3));
        double targetTravelAngle = targetAngularVelocity * transferTime;

        double optimalPhaseAngle = Math.PI - targetTravelAngle;
        while (optimalPhaseAngle < 0) optimalPhaseAngle += 2 * Math.PI;

        // Calculate wait time for optimal phasing
        double phaseAngleDifference = optimalPhaseAngle - currentPhaseAngle;
        if (phaseAngleDifference < 0) phaseAngleDifference += 2 * Math.PI;

        double spacecraftAngularVelocity = startVel.magnitude() / r1;
        ...
        return new Trajectory(startPos, predictedTargetPos, totalDeltaV,
                totalMissionTime, totalFuel, totalDeltaV);
    }
     ...
     */
    private static Vector3D predictOrbitalPosition(OrbitInitializer orbitStart,
                                                   double timeElapsed,
                                                   OrbitalSystem system) {
    ...
    public static double calculateNextLaunchWindow(Vector3D startPos, Vector3D startVel,
        ...
        double targetAngle = Math.atan2(targetPos.getY(), targetPos.getX());
        double currentPhaseAngle = targetAngle - spacecraftAngle;
        while (currentPhaseAngle < 0) currentPhaseAngle += 2 * Math.PI;
