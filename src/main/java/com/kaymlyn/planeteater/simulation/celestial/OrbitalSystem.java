...
public class OrbitalSystem {
     ...
     */
    // Add to OrbitalSystem.java (or update existing stepVerlet method)

    /**
     * UPDATED: Verlet integration step with simplified spacecraft handling.
     *
     * Spacecraft execute scheduled burns automatically when their execution
     * time arrives. No telemetry, no trajectory prediction - just burns.
     *
    ...
    public double stepVerlet() {
        ...
        // Calculate current accelerations for all orbiters
        ...
        // SIMPLIFIED: Check for launches from docked spacecraft
        for (Orbiter orbiter : orbiters.values()) {
            if(orbiter instanceof Dockable dockable) {
        ...
        // SIMPLIFIED: Process spacecraft in transit (execute scheduled burns)
    ...
    public static Orbit generateRandomOrbit(double minimumAURadius,
        ...
        return new Orbit(
                ...
                random.nextDouble()*2*Math.PI,
                random.nextDouble()*2*Math.PI,
                centerBody // initializer record, rooted at epoch
        );
    }
